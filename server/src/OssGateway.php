...
final class OssGateway
{
    ...
    public function upload(string $file, string $object, string $contentType = 'application/octet-stream'): array
    {
        ...
        $date = gmdate('D, d M Y H:i:s \G\M\T'); $resource = '/' . $bucket . '/' . str_replace('%2F', '/', rawurlencode($key));
        $stringToSign = "PUT\n\n{$contentType}\n{$date}\n{$resource}";
        ...
        if ($body === false || $status < 200 || $status >= 300) throw new RuntimeException('oss_upload_failed' . ($error ? ':' . $error : ':' . $status));
